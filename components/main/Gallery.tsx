...
const Gallery = (dimension: { width: number; height: number }) => {
  ...
  return (
    <section id="gallery" className="flex flex-col gap-20">
      <h2 className="text-left text-9xl text-textColor font-semibold px-[5rem]">
        gallery
      </h2>
      <div
        ref={container}
        className="h-[175vh] overflow-hidden bg-primaryColor"
      >
        <div className="relative top-[-12.5vh] h-[200vh] flex gap-[2rem]">
          <Column
            style={"top-[-25%]"}
...
const Column = ({
  ...
  return (
    <motion.div
      className={`relative h-full w-1/4 min-w-[250px] flex flex-col gap-[2vw] whitespace-nowrap ${style}`}
      style={{ y }}
    >
